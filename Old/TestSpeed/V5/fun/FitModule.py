...
class FitFun:
    ...
    def fit_function(self, x, *params):
        ...
        try:
            ...
            print(f"[Cycle {self.fitting_cycle}] Params: {params}")
    ...
    Fitting of the model
                ...
                print(f"Initial fitting parameters: {fitted_parameters}")
                ...
                print(f"Updated fitting parameters: {fitted_parameters}")
            ...
            if not self.ShowFullModel:
                ...
                plt.plot(xfit, OnState, label='Fitted ON state')
                plt.plot(xfit, OffState, label='Fitted OFF state')
                plt.plot(xfit, DarkState, label='Fitted DARK state')
                
                plt.xlabel('Time [s]')
                plt.ylabel('% of each population')
                if self.Use_Start_Values:
                    plt.title('Fitted model from start values')
                else:
                    plt.title('Fitted model from fitted values')
