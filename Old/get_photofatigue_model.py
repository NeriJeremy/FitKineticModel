...
from check_lasers_on import check_lasers_on
from get_state_evolution import get_state_evolution
...
########################### Calculate the illumination sequence for light induced processes ############################################################    
    ...
    try:
        ...
        Seq=0 #Overall index of the sequence (Number of time the sequence is repeated)
        ...
        #Create an array that contains ligh induced rates and S for each phase
        K_L_SEQ = [{'K_L': [], 'Seq': []} for _ in range(Switch_params['n_phases'])]
        
        for k in range(Switch_params['n_phases']):            
            i = phase_order[k]  # Set correct phase
            dc = lasers_dc[:, k]  # The lasers' duty cycles during the selected phase
            N = n_points[i-1]  # The number of points during the phase
            phase_duration = dt * N  
            
            # Find the first laser with duty cycle neither 0 nor full duration
            # w_frametime is to put None on a laser if it's On during the whole phase or to retain the index of the lasers that are partially On during the phase
            w_frametime = next((idx for idx, val in enumerate(dc) if val != 0 and val != 1), None) #phase_duration
            
            if w_frametime is None:  # If All lasers are either on or off during the entire phase
                L_ON = check_lasers_on(lasers_phi, lasers_dc, i, 0, phase_duration)
                K_L_SEQ[k]['K_L'] = np.sum(K_l[:, :, L_ON > 0], axis=2) #Select the K_l array that correspond to the On laser during this phase
                K_L_SEQ[k]['Seq'] = 1 #Seq=1 because it's the same K_l all other the phase (because the laser is constant during the phase)
            
            else:  # If Some lasers change state during the phase
                K_L_SEQ[k]['K_L'] = np.zeros((K_th.shape[0], K_th.shape[1], N)) #Here we create one slice for each k_th, because if lasers are off, thermal relaxation can occur
                
                for j in range(N):
                    t = j * dt  # Starting time                  
                    # Check if lasers are on
                    L_ON = check_lasers_on(lasers_phi, lasers_dc, i, t, N * dt)                                    
                    K_L_SEQ[k]['K_L'][:, :, j] = np.sum(K_l[:, :, L_ON > 0], axis=2)
...
########################### Perform the calculation through all cycles #############################################################
    ...
    try:
        ...
        for k in tqdm(range(1, Switch_params['n_cycle'] + 1), desc="Processing cycles"): # Go through all cycles
            
            for j0 in range(1, N_ON_OFF + 1): # Go through ON_OFF
                
                for i0 in range(1, Switch_params['n_phases'] // 2 + 1): # Go through ON_OFF phases only

                    i = phase_order[i0 - 1] # Set correct phase
                    ...
                    if N > 0: #Continue till there is no more point in the selected phase
                        ...
                        for j in range(1, N + 1):
                            ...
                            if K_L_SEQ[i0 - 1]['Seq'] == 1: #If there is a constant rate matrix during the phase (see K_L_SEQ matrix)
                                K = K_th + K_L_SEQ[i0 - 1]['K_L'] # Add thermal relaxation rate to K_L_SEQ
                                
                            else:
                                K = K_th + K_L_SEQ[i0 - 1]['K_L'][:, :, j - 1] # Add K_th to each constant rate matrix of the phase
                                
                            S[:,Seq + 1] = get_state_evolution(S[:,Seq], K, dt) # Apply kinetic scheme
                            
                            Seq += 1
                            
                            
            for j0 in range(1, N_OFF_ON + 1): # Go through OFF_N
                
                for i0 in range(Switch_params['n_phases'] // 2 + 1, Switch_params['n_phases'] + 1): # Go through OFF_ON phases only
                    
                    i = phase_order[i0 - 1] # Set correct phase
                    
                    N = n_points[i-1] # Put in N the number of point of the selected phase
