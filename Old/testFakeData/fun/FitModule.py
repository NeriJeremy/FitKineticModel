...
class FitFun:
    ...
    def Open_csv_fit(self):
        ...
        # Find the .csv files
        csv_files = [file for file in files if file.endswith('.txt')]
        ...
        if csv_files:
            ...
            if len(csv_files) == 1:   
                ...
                xtofit = self.x_scale_factor * df['X']
                ytofit = df['Y']
    ...
    Fitting of the model
                ...
                lower_b = np.zeros(len(self.w_variables))
                upper_b = np.full(len(self.w_variables), np.inf) # np.inf to not put any upper bound

                # Fit the model
                popt, pcov = curve_fit(lambda x, *params: self.fit_function(params, x), np.zeros_like(ydata),
                                       ydata, p0=startvalues, bounds=(lower_b, upper_b))
                ...
                """
                fitted_parameters has to correspond to Fit_params
                ...
                """
                error probably because np.copy, we have to copy it has a dico !
