...
get_photofatigue_model_count = 0

def get_photofatigue_model(rate_array, Switch_params, start):
    
    try:
        
        #To count the number of time the function has been called and stop some useless prints
        global get_photofatigue_model_count
        get_photofatigue_model_count += 1
        ...
        if get_photofatigue_model_count <= 1:
...
############################ Calculate number of dt and time steps ################################################################################# 
    ...
    try:
        ...
        for i in range(Switch_params['n_phases']):
            K = K_th.copy()
            # Iterate over lasers
            for j in range(Switch_params['n_lasers']):
                # lasers_on[j] gives the status of the j-th laser for phase i
                laser_status = lasers_on[j, i]
                
                # Multiply K_l by the laser status for the current phase
                K += K_l[:, :, j] * laser_status
        ...
        n_frames = math.floor(Switch_params['n_cycle'] * (Switch_params['T_ON-OFF'] + Switch_params['T_OFF-ON']) / 
                              (Switch_params['Frametime'] + Switch_params['AddTime']))
        if get_photofatigue_model_count <= 1:
...
#################################### Define concentrations for each states ############################################################
    ...
    try:
        ...
        Dark_frac = max(0, 1.0 - (start[4] + start[5])) # Calculate the Dark fraction
        
        S[0,0] = start[3] * start[4] # ['On_init']
        S[1,0] = start[3] * start[5] # ['Off_init']
        S[2,0] = start[3] * Dark_frac # ['Dark_init']
...
########################################################### Define Phases ############################################################
    ...
    try:
        ...
        #Set phase order
        phase_order = np.arange(Switch_params['n_phases'])
...
########################################## Compute the fluorescent signal #############################################################
    ...
    try:
        ...
        DC_read = DC[read_frames, :]  # Select rows corresponding to read_frames
        ...
        IND2 = np.arange(n_frames)  # Create an array with total number of frames
        ...
        FL = start[3] * start[4] * FL / FL[0]  # Rescale with fluo signal
